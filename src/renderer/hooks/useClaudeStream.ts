...
const useClaudeStream = (options?: Options | null): Result => {
	...
	const normalized = useMemo(() => {
		...
		return {
			...
			customClaudeConfigId: options.customClaudeConfigId,
		};
	}, [
		options?.workspaceId,
		options?.workspacePath,
		options?.customClaudeConfigId,
	]);
		...
		async (text: string, attachments: string = "") => {
			...
			try {
				await window.electronAPI.agentSendMessageStream({
					...
					conversationId: convoId,
					customClaudeConfigId: normalized.customClaudeConfigId,
				});
				return { success: true };
			} catch (e: any) {
	...
	const cancel = useCallback(async () => {
		...
		try {
			...
			if (!res?.success)
				return { success: false, error: res?.error || "stop-stream-failed" };
			setIsStreaming(false);
			setSeconds(0);
			return { success: true };
		} catch (e: any) {
	...
	useEffect(() => {
		...
		const loadConversation = async () => {
			...
			try {
				const convo =
					await window.electronAPI.getOrCreateDefaultConversation(wid);
				if (cancelled) return;
				if (convo?.success && convo.conversation) {
					setConversationId(convo.conversation.id);
					const msgs = await window.electronAPI.getMessages(
		...
		const offDone = window.electronAPI.onAgentStreamComplete(
			async (data: any) => {
				if (data.providerId !== "claude") return;
				if (data.workspaceId !== wid) return;
				setIsStreaming(false);
