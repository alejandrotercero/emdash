...
export function registerDatabaseIpc() {
	...
	ipcMain.handle(
		...
		async (_, workspaceId: string) => {
			try {
				const conversation =
					await databaseService.getOrCreateDefaultConversation(workspaceId);
	...
	// Custom Claude config management
	ipcMain.handle("db:saveCustomClaudeConfig", async (_, config: any) => {
		try {
			await databaseService.saveCustomClaudeConfig(config);
			return { success: true };
		} catch (error) {
			log.error("Failed to save custom Claude config:", error);
			return { success: false, error: (error as Error).message };
		}
	});

	ipcMain.handle("db:getCustomClaudeConfigs", async () => {
		try {
			const configs = await databaseService.getCustomClaudeConfigs();
			return { success: true, configs };
		} catch (error) {
			log.error("Failed to get custom Claude configs:", error);
			return { success: false, error: (error as Error).message };
		}
	});

	ipcMain.handle("db:getCustomClaudeConfig", async (_, id: string) => {
		try {
			const config = await databaseService.getCustomClaudeConfig(id);
			return { success: true, config };
		} catch (error) {
			log.error("Failed to get custom Claude config:", error);
			return { success: false, error: (error as Error).message };
		}
	});

	ipcMain.handle("db:deleteCustomClaudeConfig", async (_, id: string) => {
		try {
			await databaseService.deleteCustomClaudeConfig(id);
			return { success: true };
		} catch (error) {
			log.error("Failed to delete custom Claude config:", error);
			return { success: false, error: (error as Error).message };
		}
	});
}
