...
contextBridge.exposeInMainWorld("electronAPI", {
	...
	onPtyStarted: (listener: (data: { id: string }) => void) => {
		...
		const wrapped = (_: Electron.IpcRendererEvent, data: { id: string }) =>
			listener(data);
		ipcRenderer.on(channel, wrapped);
		return () => ipcRenderer.removeListener(channel, wrapped);
	},
	...
	// Linear integration
	linearSaveToken: (token: string) =>
		ipcRenderer.invoke("linear:saveToken", token),
	linearCheckConnection: () => ipcRenderer.invoke("linear:checkConnection"),
	linearClearToken: () => ipcRenderer.invoke("linear:clearToken"),
	linearInitialFetch: (limit?: number) =>
		ipcRenderer.invoke("linear:initialFetch", limit),
	...
	saveCustomClaudeConfig: (config: any) =>
		ipcRenderer.invoke("db:saveCustomClaudeConfig", config),
	getCustomClaudeConfigs: () => ipcRenderer.invoke("db:getCustomClaudeConfigs"),
	getCustomClaudeConfig: (id: string) =>
		ipcRenderer.invoke("db:getCustomClaudeConfig", id),
	deleteCustomClaudeConfig: (id: string) =>
		ipcRenderer.invoke("db:deleteCustomClaudeConfig", id),
